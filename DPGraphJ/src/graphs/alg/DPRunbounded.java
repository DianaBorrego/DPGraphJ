...
public class DPRunbounded<V, E, S> {
	
	public static <V, E, S> DPRunbounded<V, E, S> ofGreedy(
			EGraph<V, E> graph) {
		GreedyOnGraph<V, E> ga = GreedyOnGraph.of(graph);
		Optional<GraphPath<V, E>> gp = ga.search();
		if(gp.isPresent()) return DPRunbounded.of(graph,null,gp.get().getWeight(),gp.get(),false);
		else return DPRunbounded.of(graph,null,null,null,false);
	}
	
	public static <V, E, S> DPRunbounded<V, E, S> of(
			EGraph<V, E> graph) {
		return DPRunbounded.of(graph,null,null,null,false);
	}
	
	public static <V, E, S> DPRunbounded<V, E, S> of(
			EGraph<V, E> graph, Function<GraphPath<V, E>, S> fsolution, Double bestValue, GraphPath<V, E> optimalPath, Boolean withGraph) {
		return new DPRunbounded<V, E, S>(graph,fsolution,bestValue,optimalPath,withGraph);
	}
	...
	DPRunbounded(EGraph<V, E> g, Function<GraphPath<V, E>, S> fsolution, Double bestValue, GraphPath<V, E> optimalPath, Boolean withGraph) {
		this.graph = g;
